...
static PyObject* vectorToTuple_Int(const std::vector<int> &data) {
    ...
    for (Py_ssize_t i = 0; i < (Py_ssize_t)data.size(); i++) {
        ...
        if (!num) {
            Py_DECREF(tuple);
            PyErr_SetString(PyExc_MemoryError, "Unable to allocate memory for Python integer");
            return NULL;
        }
        PyTuple_SET_ITEM(tuple, i, num); // steals reference to num
    }
    return tuple;
}
...
static void *pyobject_to_void(PyObject *obj) {
    if (!obj) {
        PyErr_SetString(PyExc_TypeError, "NULL PyObject passed");
        return NULL;
    }
    if (!PyCapsule_CheckExact(obj)) {
        PyErr_SetString(PyExc_TypeError, "Expected a PyCapsule (internal pointer)");
        return NULL;
    }
    void *ptr = PyCapsule_GetPointer(obj, NULL);
    if (!ptr && PyErr_Occurred()) {
        // PyCapsule_GetPointer a déjà posé une erreur
        return NULL;
    }
    return ptr;
}
...
if (!PyArg_ParseTuple(args, "L", &val)) {
    PyErr_Format(PyExc_TypeError, "The argument must be an integer representing the number of classes");
    return NULL;
}
...
pyxai::Rectifier *rectifier = (pyxai::Rectifier *) pyobject_to_void(class_obj);
if (!rectifier) return NULL;  // pyobject_to_void a posé une erreur

if (id_tree_obj < 0 || id_tree_obj >= (int)rectifier->trees.size()) {
    PyErr_SetString(PyExc_IndexError, "tree id out of range");
    return NULL;
}
if (!rectifier->trees[id_tree_obj]) {
    PyErr_SetString(PyExc_RuntimeError, "Null tree pointer");
    return NULL;
}

PyObject *res = rectifier->trees[id_tree_obj]->toTuple();
if (!res) {
    // toTuple() should set an exception on failure
    return NULL;
}
return res;
...
static PyObject *set_excluded(PyObject *self, PyObject *args) {
    ...
    for(int i = 0; i < size_obj; i++) {
        PyObject *value_obj = PyTuple_GetItem(vector_excluded_obj, i); // borrowed
if (!value_obj) {
    PyErr_SetString(PyExc_RuntimeError, "Failed to get tuple item");
    return NULL;
}
if (!PyLong_Check(value_obj)) {
    PyErr_SetString(PyExc_TypeError, "Expected integer in tuple");
    return NULL;
}
long v = PyLong_AsLong(value_obj);
if (PyErr_Occurred()) return NULL;
explainer->excluded_features.push_back((int)v);
...
static PyObject *compute_reason(PyObject *self, PyObject *args) {
    ...
    // Get pointer to the class
    pyxai::Explainer *explainer = (pyxai::Explainer *) pyobject_to_void(class_obj);
    if (!explainer) return NULL;  // pyobject_to_void a déjà mis l'erreur
...
if (ret == false) {
    Py_RETURN_NONE;   // macro sûre : Py_INCREF(Py_None); return Py_None;
}

// convertir reason -> PyObject*
PyObject *py_reason = nullptr;
try {
    py_reason = vectorToTuple_Int(reason);
