...
class BitSet:
    ...
    def __init__(self, n_bits, elements):
        self.n_bits = n_bits
        self.__bits = [self.__ALL_CLEAR for _ in range(self.__number_of_integers_needed(n_bits))]
        for elt in elements: 
            self.set(elt, 1)
        #print("Nb integers = ", len(self.__bits))


    def issubset(self, mysubbitset): 
        for i in range(len(self.__bits)):
                tmp = self.__bits[i] & mysubbitset.__bits[i]
                if tmp != self.__bits[i]: 
                    return False
        return True
    ...
    def __len__(self): 
        tmp = 0
        for i in range(len(self.__bits)): 
            tmp += self.__bits[i].bit_count()
        return tmp
    
    def union(self, other): 
        tmp = BitSet(self.n_bits, [])
        for i in range(len(self.__bits)): 
            tmp.__bits[i] = self.__bits[i] | other.__bits[i]
        return tmp
